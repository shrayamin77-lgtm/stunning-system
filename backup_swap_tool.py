...
def load_data():
    ...
    return backup_df, matrix_df

backup_df, matrix_df = load_data()
...
def is_on_elective(resident_name, range_str):
    ...
    if not matrix_col: return True  # Default fallback if block interval is unmapped
...
if user_ranges:
    ...
    if st.button("🔎 Search Backup Swaps"):
        ...
        if eligible_swaps:
            ...
            st.table(pd.DataFrame(eligible_swaps))
        else:
